...
`CHANGELOG.md` is auto-generated by [release-plz](https://github.com/release-plz/release-plz) from conventional commit subjects (see `.release-plz.toml` + `.github/workflows/coordinode-release.yml`). Manual edits in feature PRs WILL BE CLOBBERED on the next release run.

**Rules:**
- NEVER edit `CHANGELOG.md` in a feature PR.
- The only commits that touch `CHANGELOG.md` are the auto-generated `chore: release vX.Y.Z` PRs opened by release-plz itself.
- To get a line into the next release notes, write a conventional commit subject (`feat(scope): ...`, `fix(scope): ...`, `perf(scope): ...`) — release-plz parses the subject and types it under Added / Fixed / Performance automatically.
- If a release entry needs richer wording than the commit subject, edit `.release-plz.toml`'s changelog template, not the generated `CHANGELOG.md`.
