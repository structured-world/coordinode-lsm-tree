...
| `bytes_copied` | Bytes moved by a **gather**: column-batch accumulation, batch filtering, row gathering by index, and row-value reconstruction from sub-columns, on every read that performs one (single-segment and merged columnar scans, row iteration and point reads of a columnar segment) and in salvage. The figure is the size of the RESULT. A row read whose value is a single bytes column hands out views into the decoded column and charges nothing. | Transform output (that is `bytes_decoded`), write-path serialisation, the input decoding of compaction and repair (maintenance, not reads), and moves that transfer ownership without duplicating bytes. |
