...
| `bytes_copied` | Bytes moved by a **gather**: column-batch accumulation, batch filtering, row gathering by index, row-value reconstruction from sub-columns, a point read's copy of the matching rows out of the columns, the row-major block a columnar block is re-encoded into, the copy of each uncompressed blob record out of a scan's read-ahead span (so a cached value does not pin the whole span), and what decoding a columnar block copies out of it (validity bitmaps, and columns a narrow projection detaches rather than keep the whole block alive for), on every read that performs one (single-segment and merged columnar scans, row iteration and point reads of a columnar segment). The figure is the size of the RESULT. A row read whose value is a single bytes column hands out views into the decoded column and charges nothing. | Transform output (that is `bytes_decoded`), write-path serialisation, the input decoding of compaction, repair and salvage (maintenance, not reads), a restricted table's live-row count (statistics), and moves that transfer ownership without duplicating bytes. |
