...
/// The tight-space restriction position must survive an in-place meta patch on an
/// ENCRYPTED table: `persist_restrict_position` decodes the AEAD-sealed meta,
/// swaps the fixed-width descriptor, and re-seals with a FRESH nonce. Because the
/// plaintext width is unchanged the re-sealed block is byte-identical in size
/// (nonce 12B + ciphertext + tag 16B) and overwrites its region cleanly; reopening
/// with the provider decrypts the patched meta and reads back the exact position.
#[cfg(feature = "encryption")]
#[test]
fn persist_restrict_position_roundtrips_through_encryption() -> crate::Result<()> {
    use crate::ValueType;
    use crate::encryption::{Aes256GcmProvider, EncryptionProvider};
    use std::sync::Arc;

    let provider: Arc<dyn EncryptionProvider> = Arc::new(Aes256GcmProvider::new(&[7u8; 32]));
    let dir = tempdir()?;
    let sst = dir.path().join("0");
    let fs: Arc<dyn crate::fs::Fs> = Arc::new(StdFs);

    let recover = |enc: Arc<dyn EncryptionProvider>| -> crate::Result<Table> {
        let checksum =
            crate::Checksum::from_raw(crate::repair::compute_table_checksum(&*fs, &sst)?);
        Table::recover(
            sst.clone(),
            checksum,
            0,
...

    {
        let mut w = Writer::new(sst.clone(), 0, 0, Arc::clone(&fs))?
            .use_encryption(Some(Arc::clone(&provider)))
            .use_data_block_size(128);
        for i in 0..256u32 {
        ...
        assert!(w.finish()?.is_some(), "the encrypted SST is non-empty");
    }

    let table = recover(Arc::clone(&provider))?;
    assert_eq!(
        table.metadata.restrict_position, None,
        "a fresh SST carries the unrestricted sentinel",
    );
    ...
    let reopened = recover(Arc::clone(&provider))?;
    assert_eq!(
        reopened.metadata.restrict_position,
        Some((bo, eo)),
        "the re-sealed encrypted meta carries the stamped position",
    );
    assert_eq!(
        reopened.restrict_bound_at(bo, eo)?.as_ref(),
        bound.as_slice(),
        "the position resolves to the exact bound after an encrypted re-seal",
    );
    Ok(())
}
